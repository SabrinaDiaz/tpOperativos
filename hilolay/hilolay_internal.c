...
// TODO: Check what happens when a thrad is closed, Comprueba lo que sucede cuando el hilo está cerrado


/* Writes administrative info of a TCB, Escribe información administrativa de un TCB */
...
void init_internal(struct hilolay_operations* ops){
	/* Saves operations for later use, Guarda operaciones para su uso posterior ,
	 OSEA DE TODAS LAS OPERACIONES QUE GUARDA DESPUES ELIGE CUAL USA */
	main_ops = ops;

	/* Initializes main as a thread, Inicializa main como un hilo */
	/* TODO: Abstract*/
    current_ult = &ults[0];
    current_ult->context = malloc(sizeof(ucontext_t));
    set_tcb(current_ult, MAIN_THREAD_ID, RUNNING);

    /* Call function on clients code, Función de llamada en código de clientes */
...
/* Creates the context of a new thread , Crea el contexto de un nuevo hilo*/
void create_context(struct TCB* new_ult, void *(*start_routine)(void *), void *arg) {
    ...
    Obtiene el contexto actual como referencia, para ser sobrescrito
    // TODO: Why current context? I assume this may affect context for global variables, but it's worth checking,

    ¿Por qué contexto actual? Supongo que esto puede afectar el contexto de las variables globales, pero vale la pena verificarlo*/
...
// TODO: Make referentially transparent,Hacer referencialmente transparente
int get_next_id(){
	return NEXT_ID++;
}
...
/* Returns the running thread id, Devuelve el id del hilo en ejecución */
